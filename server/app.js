...
// We instantiate an `express()` application into the `app` variable.
...
// We set this app (through _middlewares_) to use sessions w/ this configuration.
// Every user requesting some endpoint from this server will be assigned to a session _space_ in the server that will be unique for every user (controller by cookies on the client).
// The data of sessions will be available on every request in `req.session`.
app.use(session({
...
// We set this app (through _middlewares_) to parse the data received from the client.
// This data will be available in `req.body`.<br/>
